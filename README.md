...
- Livelier low-frequency idle loop: breathing, head/body bob, chest-height claw lifts, and a gentle heel/toe weight shift.
  更有生命力的低频待机循环：呼吸、头部/身体轻摆、抬至胸前的手爪收放，以及轻微抬跟/脚尖重心变化。
...
- Needs-user-input alert: a repeated two-arm wave and jump loop for approvals, permissions, answers, and decisions.
  需要用户介入提醒：当等待批准、权限、回答或选择时，持续循环双臂挥舞与跳跃动作。
...
| Needs input — alert jump / 需要介入—提醒跳跃 | ![Needs input / 需要介入](previews/waiting.gif) |
...
For the `Needs input` alert behavior, see [`docs/RUNTIME_BEHAVIOR.md`](docs/RUNTIME_BEHAVIOR.md).

关于“需要输入”提醒的运行时行为，详见 [`docs/RUNTIME_BEHAVIOR.md`](docs/RUNTIME_BEHAVIOR.md)。
